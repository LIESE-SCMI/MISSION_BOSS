...
void UART0_Handler(void) {
    ...
    PC_DataReceived_SCMI_SUBSYSTEM = (UART0_DR_R & UART_DR_DATA_M);
    PC_DataReceived_SUBSYSTEM_ID = (UART0_DR_R & UART_DR_DATA_M);
    while (UART0_FR_R & UART_FR_RXFE) {}
    PC_DataReceived_COMMAND_ID = (UART0_DR_R & UART_DR_DATA_M);
    while (UART0_FR_R & UART_FR_RXFE) {}
    PC_DataReceived_DATA_SIZE = (UART0_DR_R & UART_DR_DATA_M);

    int i = 0;
    for (i = 0; i < PC_DataReceived_DATA_SIZE; i++) {
        while (UART0_FR_R & UART_FR_RXFE) {}
        PC_DataReceived_DATA[i] = (UART0_DR_R & UART_DR_DATA_M);
    }

    while (UART0_FR_R & UART_FR_RXFE) {}
    PC_DataReceived_CRC_MSB = (UART0_DR_R & UART_DR_DATA_M);
    while (UART0_FR_R & UART_FR_RXFE) {}
    PC_DataReceived_CRC_LSB = (UART0_DR_R & UART_DR_DATA_M);
    while (UART0_FR_R & UART_FR_RXFE) {}
    PC_DataReceived_STOP_BYTE = (UART0_DR_R & UART_DR_DATA_M);
...
void UART4_Handler(void) {
    ...
    /** Validación del comando recibido */
    SUPERVISOR_DataReceived_SUBSYSTEM_ID = (UART4_DR_R & UART_DR_DATA_M);
    SUPERVISOR_DataReceived_COMMAND_ID = (UART4_DR_R & UART_DR_DATA_M);
    while (UART4_FR_R & UART_FR_RXFE) {}
    SUPERVISOR_DataReceived_DATA_SIZE = (UART4_DR_R & UART_DR_DATA_M);

    int i = 0;
    for (i = 0; i < SUPERVISOR_DataReceived_DATA_SIZE; i++) {
        while (UART4_FR_R & UART_FR_RXFE) {}
        SUPERVISOR_DataReceived_DATA[i] = (UART4_DR_R & UART_DR_DATA_M);
    }

    while (UART4_FR_R & UART_FR_RXFE) {}
    SUPERVISOR_DataReceived_CRC_MSB = (UART4_DR_R & UART_DR_DATA_M);
    while (UART4_FR_R & UART_FR_RXFE) {}
    SUPERVISOR_DataReceived_CRC_LSB = (UART4_DR_R & UART_DR_DATA_M);
    while (UART4_FR_R & UART_FR_RXFE) {}
    SUPERVISOR_DataReceived_STOP_BYTE = (UART4_DR_R & UART_DR_DATA_M);
    ...
    uint8_t j;
    while (UART4_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (SUPERVISOR_DataReceived_SUBSYSTEM_ID & UART_DR_DATA_M);
    while (UART4_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (SUPERVISOR_DataReceived_COMMAND_ID & UART_DR_DATA_M);
    while (UART4_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (SUPERVISOR_DataReceived_DATA_SIZE & UART_DR_DATA_M);

    for (j = 0; j < SUPERVISOR_DataReceived_DATA_SIZE; j++) {
        while (UART4_FR_R & UART_FR_TXFF) {}
        UART0_DR_R = (SUPERVISOR_DataReceived_DATA[j] & UART_DR_DATA_M);
    }

    while (UART4_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (SUPERVISOR_DataReceived_CRC_MSB & UART_DR_DATA_M);
    while (UART4_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (SUPERVISOR_DataReceived_CRC_LSB & UART_DR_DATA_M);
    while (UART4_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (SUPERVISOR_DataReceived_STOP_BYTE & UART_DR_DATA_M);
}
...
void UART7_Handler(void) {
    ...
    /** Validación del comando recibido */
    MASTER_DataReceived_SUBSYSTEM_ID = (UART7_DR_R & UART_DR_DATA_M);
    MASTER_DataReceived_COMMAND_ID = (UART7_DR_R & UART_DR_DATA_M);
    while (UART7_FR_R & UART_FR_RXFE) {}
    MASTER_DataReceived_DATA_SIZE = (UART7_DR_R & UART_DR_DATA_M);

    int i = 0;
    for (i = 0; i < MASTER_DataReceived_DATA_SIZE; i++) {
        while (UART7_FR_R & UART_FR_RXFE) {}
        MASTER_DataReceived_DATA[i] = (UART7_DR_R & UART_DR_DATA_M);
    }

    while (UART7_FR_R & UART_FR_RXFE) {}
    MASTER_DataReceived_CRC_MSB = (UART7_DR_R & UART_DR_DATA_M);
    while (UART7_FR_R & UART_FR_RXFE) {}
    MASTER_DataReceived_CRC_LSB = (UART7_DR_R & UART_DR_DATA_M);
    while (UART7_FR_R & UART_FR_RXFE) {}
    MASTER_DataReceived_STOP_BYTE = (UART7_DR_R & UART_DR_DATA_M);
    ...
    uint8_t j;
    while (UART7_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (MASTER_DataReceived_SUBSYSTEM_ID & UART_DR_DATA_M);
    while (UART7_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (MASTER_DataReceived_COMMAND_ID & UART_DR_DATA_M);
    while (UART7_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (MASTER_DataReceived_DATA_SIZE & UART_DR_DATA_M);

    for (j = 0; j < MASTER_DataReceived_DATA_SIZE; j++) {
        while (UART7_FR_R & UART_FR_TXFF) {}
        UART0_DR_R = (MASTER_DataReceived_DATA[j] & UART_DR_DATA_M);
    }

    while (UART7_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (MASTER_DataReceived_CRC_MSB & UART_DR_DATA_M);
    while (UART7_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (MASTER_DataReceived_CRC_LSB & UART_DR_DATA_M);
    while (UART7_FR_R & UART_FR_TXFF) {}
    UART0_DR_R = (MASTER_DataReceived_STOP_BYTE & UART_DR_DATA_M);
}
